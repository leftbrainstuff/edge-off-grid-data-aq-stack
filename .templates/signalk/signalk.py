...
def main():
  ...
  def mainRender(menu, selection):
    ...
    print(term.black_on_cornsilk4(term.center('IOTstack Signalk Commands')))
    print("")
  ...
  def runSelection(selection):
    ...
    if len(menuItemsExample[selection]) > 1 and isinstance(menuItemsExample[selection][1], types.FunctionType):
      ...
      print(term.green_reverse('IOTstack Signalk Error: No function assigned to menu item: "{}"'.format(menuItemsExample[selection][0])))
...
global currentServiceName # Name of the current service
if currentServiceName == 'signalk':
  main()
else:
  print("Error. '{}' Tried to run 'signalk' config".format(currentServiceName))
