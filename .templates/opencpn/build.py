...
def main():
  ...
  documentationHint = 'https://sensorsiot.github.io/IOTstack/Containers/Opencpn' #TODO redirect
  ...
  def onResize(sig, action):
    global opencpnBuildOptions
    global currentMenuItemIndex
    mainRender(1, opencpnBuildOptions, currentMenuItemIndex)
  ...
  def mainRender(needsRender, menu, selection):
    ...
    if needsRender == 1:
      ...
      print(term.black_on_cornsilk4(term.center('IOTstack opencpn Options')))
      print("")
  ...
  def runSelection(selection):
    import types
    global opencpnBuildOptions
    if len(opencpnBuildOptions[selection]) > 1 and isinstance(opencpnBuildOptions[selection][1], types.FunctionType):
      opencpnBuildOptions[selection][1]()
    else:
  ...
  def menuEntryPoint():
    ...
    with term.fullscreen():
      menuNavigateDirection = 0
      mainRender(needsRender, opencpnBuildOptions, currentMenuItemIndex)
      selectionInProgress = True
      with term.cbreak():
        while selectionInProgress:
          ...
          if menuNavigateDirection != 0: # If a direction was pressed, find next selectable item
            ...
            currentMenuItemIndex = currentMenuItemIndex % len(opencpnBuildOptions)
            needsRender = 2

            while not isMenuItemSelectable(opencpnBuildOptions, currentMenuItemIndex):
              currentMenuItemIndex += menuNavigateDirection
              currentMenuItemIndex = currentMenuItemIndex % len(opencpnBuildOptions)
    return True
...
global currentServiceName # Name of the current service
if currentServiceName == 'opencpn':
  main()
else:
  print("Error. '{}' Tried to run 'opencpn' config".format(currentServiceName))
